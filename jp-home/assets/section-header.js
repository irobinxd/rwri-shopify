	...
	class MainHeader extends HTMLElement {
		...
		mount(){
			...
			if (this.hasAttribute('data-sticky-header')) {
				...
				const announcementContainer = announcement.closest('announcement-bar') || announcement.parentElement.parentElement;

				// Add fixed classes
				announcementContainer.classList.add('announcement-bar-fixed');
				...
				const updatePositions = () => {
					...
					if (st > lastScroll && st > 50) {
						...
						// Only adjust header position when at the very top
						if (st <= 0) {
							header.style.top = announcement.offsetHeight + "px";
						} else {
							header.style.top = "0px";
						}
					}
				...
				// Set header offset initially
				const setInitialOffset = () => {
					header.style.top = announcement.offsetHeight + "px";
					document.body.style.paddingTop =
					announcement.offsetHeight + header.offsetHeight + "px";
				};
