	...
	class CartForm extends HTMLElement {
		...
		ajaxifyCartItems(){
			...
			this.querySelectorAll('[data-js-cart-item]').forEach(item=>{
				...
				if ( qty ) {// Prevent quantity changes for freebie items
					if (item.classList.contains('cart-item--freebie')) {
						qty.disabled = true;
						qty.value = 1;
					} else {
		...
		updateCartQty(item, newQty){
			...
			let alert = null;
			// Prevent quantity changes for freebie items - keep at 1
			if (item.classList.contains('cart-item--freebie')) {
				if (newQty !== 1 && newQty !== 0) {
					// If trying to change freebie quantity (not remove), prevent it
...
if ( typeof CartProductQuantity !== 'function' ) {
	...
	class CartProductQuantity extends HTMLElement {
		...
		changeCartInput(){
			const cartItem = this.closest('[data-js-cart-item]');
			// Prevent quantity changes for freebie items
			if (cartItem && cartItem.classList.contains('cart-item--freebie')) {
			...
			setTimeout(()=>{
				document.getElementById('AjaxCartForm').updateCartQty(cartItem, parseInt(this.querySelector('.qty').value));
			}, 50);
		}
	}
...
if ( ! window.refreshCart ) {
	...
	window.refreshCart = () => {
		...
		fetch('?section_id=helper-cart')
			...
			// Re-initialize freebie buttons after ajax refresh (defined in cart-form-page)
			if ( typeof window.initFreebieHandlers === 'function' ) {
				setTimeout(() => window.initFreebieHandlers(), 50);
			}
...
// Wine Gift Option - Updates cart attributes
if ( ! customElements.get('wine-gift-option') ) {
  customElements.define(
    'wine-gift-option',
    class WineGiftOption extends HTMLElement {
      constructor() {
        super();
        ...
        if (checkbox) {
          checkbox.addEventListener('change', (event) => {
            const attributes = {
              "Gift Option": event.target.checked ? "Yes" : ""
            };
